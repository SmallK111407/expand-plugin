...
class GsCfg {
  constructor () {
    /** 默认设置 */
    this.defSetPath = './plugins/expand-plugin/defSet/'
    this.defSet = {}

    /** 用户设置 */
    this.configPath = './plugins/expand-plugin/config/'
    this.config = {}
  ...
  cpCfg (app, name) {
    ...
    let set = `./plugins/expand-plugin/config/${app}.${name}.yaml`
    if (!fs.existsSync(set)) {
      fs.copyFileSync(`./plugins/expand-plugin/defSet/${app}/${name}.yaml`, set)
    }
  }
